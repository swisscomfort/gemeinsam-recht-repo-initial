# GEMEINSAM RECHT — MANIFEST v2.1

**Bindende Fassung · Stand 8. August 2026**
Ersetzt: MANIFEST v2.0 (SHA-256 f10db698…373652)
Vorgänger: Plan v1.1 (SHA-256 4e42ba6d…9aa906)
Grundlage: Konzeptfassung v2.0, geändert durch CR-01 (Doppelkodierung)

**Änderung gegenüber v2.0:** §3 Feldliste und §5 Absatz 2 — Handprüfung ersetzt durch
Doppelkodierung. Alle übrigen Paragraphen wortgleich.
...
| `kodierung_status` | vorschlag \| doppelt_bestaetigt \| mensch_bestaetigt \| strittig — Default `vorschlag` |
| `kodierung_quellen` | Liste der Kodierläufe: Bezeichnung, Datum, vergebener Wert, Textstelle |
...
- zusätzlich für die Scheiterpunkt-Auswertung: `kodierung_status` ist `vorschlag`
  oder `strittig`

**Ein Scheiterpunkt ist eine Deutung, keine Tatsache aus dem Urteil.**

Gezählt wird ausschliesslich, was `doppelt_bestaetigt` oder `mensch_bestaetigt`
trägt. `doppelt_bestaetigt` setzt zwei voneinander unabhängige Kodierläufe voraus,
die denselben Wert vergeben und je eine Textstelle als Beleg führen. Unabhängig
heisst: **verschiedene Modelle, nicht derselbe Lauf zweimal.**

Weichen die Läufe ab, erhält der Fall `strittig`. Strittige Fälle sind von jeder
Zählung ausgeschlossen und werden zur menschlichen Entscheidung ausgewiesen.

Zu jedem gezählten Wert bleibt abrufbar, aus welchen Läufen er stammt.

Jede Zählung gibt aus: Zähler, Nenner, Zahl der ausgeschlossenen Fälle je
Ausschlussgrund, sowie die **Übereinstimmungsquote der Kodierläufe**.
